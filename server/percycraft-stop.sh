...
stop-server() {
    ...
    /usr/local/bin/docker-compose -f /opt/percycraft/profile/$PROFILE/docker-compose.yml --env-file /opt/data/percycraft.env down
    echo Stop server complete >&2
}

backup() {
    if [ "$IS_TEST_RUN" ]; then
        echo Backup skipped >&2
    else
...
generate-image() {
    if [ "$IS_TEST_RUN" ]; then
        echo Generating image skipped >&2
    else
